...
function getAllImages(z: Ziel): { src: string; alt: string }[] {
  const images: { src: string; alt: string }[] = []
  
  if (z.bilder?.length > 0) {
    images.push(...z.bilder.map(bild => ({
      src: `${config.public.apiBase.replace('/api/v1', '')}/media/bilder/${bild.id}`,
      alt: z.name
    })))
  }
  
  // Add category images as fallback if no images
  if (images.length === 0 && z.kategorien?.length > 0) {
    for (const kat of z.kategorien) {
      if (kat.bild_data?.id) {
        images.push({
          src: `${config.public.apiBase.replace('/api/v1', '')}/media/bilder/${kat.bild_data.id}`,
          alt: kat.name
  ...
  <article v-else-if="ziel" class="animate-fade-in">
    ...
    <div class="mb-8 animate-slide-up" style="animation-delay: 100ms">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div
          class="md:col-span-2 aspect-video rounded-lg overflow-hidden bg-muted group relative cursor-pointer"
        ...
        >
          ...
          <div class="absolute inset-0 flex items-center justify-center bg-black/0 group-hover:bg-black/30 transition-colors">
            <svg class="h-12 w-12 text-white opacity-0 group-hover:opacity-100 transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div v-if="ziel.bilder?.length > 1" class="grid grid-cols-2 gap-4">
          <div
            ...
            :key="bild.id"
            class="aspect-square rounded-lg overflow-hidden bg-muted group relative cursor-pointer"
          ...
          >
            ...
            <div
              v-if="index === 3 && ziel.bilder.length > 5"
              class="absolute inset-0 bg-black/60 flex items-center justify-center"
            >
              <span class="text-white text-xl font-bold">+{{ ziel.bilder.length - 5 }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- Lightbox -->
    <ImageLightbox
      v-if="lightboxOpen && getAllImages(ziel).length > 0"
      :images="getAllImages(ziel)"
